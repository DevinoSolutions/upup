...
| Headless core           | Yes (a hook) — but file selection only, no uploading      | Yes (`@upup/core`) — a full upload engine                        |
| License / pricing       | MIT, free & open source                                   | MIT, free & open source                                          |
| Uploading / self-host   | You implement uploads yourself, to any backend            | `@upup/server` presigns and proxies to any S3-compatible storage |
