...
The server→S3 multipart cutoff is fixed and not configurable: files up
to 5 MiB stream through as a single PUT; larger files use S3 multipart
with 5 MiB chunks. Server memory envelope is one chunk at a time
regardless of file size — the old `multipartThreshold` knob was removed
so the memory bound cannot be raised away by configuration.
