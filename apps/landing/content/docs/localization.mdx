...
In the [headless](/docs/guides/headless/) path — `useUpupUpload` or `UpupCore` —
...
User-facing error copy goes through the same system: every
[`UpupErrorCode`](/docs/api-reference/error-codes/) maps to a key in the
`errors` namespace (via `errorCodeToMessageKey` in `@useupup/core/i18n`), so a
localized bundle — or a targeted `overrides.errors` entry — changes what users
see when an upload is rejected or fails. Override the `errors` namespace like
any other; unknown codes fall back to a generic message.

## Next steps

- [Theming](/docs/guides/theming/) — restyle the components whose copy you just
  localized.
- [Headless Usage](/docs/guides/headless/) — set the `locale` option on the engine
