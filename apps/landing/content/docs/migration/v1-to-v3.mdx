...
<Callout type="note">
    ...
    storage-provider type, `@upupjs/react` re-exports `StorageProvider` from
    `@upupjs/core`.
</Callout>
...
- **What v3 server mode adds:** the client talks to your `serverUrl` for OAuth
  and drive access, and `@upupjs/server` performs the OAuth exchange and
...
- **What stays the same for storage:** both modes still upload file bytes
  directly to storage via a presigned URL. In server mode, `@upupjs/server`
...
With `@upupjs/server`, flat policy is declarative and per-user policy goes in the
