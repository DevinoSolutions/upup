...
Monitoring](/docs/guides/error-monitoring/) for the codes you'll tag most often.
...
You can configure automatic retries for failed uploads using the [`maxRetries`](/docs/api-reference/upupuploader/optional-props/#maxretries) prop on the `UpupUploader` component. When set, each file upload is silently retried up to the specified number of times before being considered a failure.
...
When [resumable multipart uploads](/docs/resumable-uploads/) are enabled (`resumable={{ protocol: 'multipart' }}`), error recovery is enhanced:
