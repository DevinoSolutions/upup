...
When [resumable multipart uploads](/docs/resumable-uploads/) are enabled (`resumable={{ protocol: 'multipart' }}`), the failure UI changes but the recovery mechanism does not:
...
Uploads](/docs/resumable-uploads/).
