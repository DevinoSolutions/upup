...
description: The stable optional UpupUploader props — sources, cloudDrives, i18n, theme, metadata, folderUpload, cors, maxRetries, animations, and resumable.
---
...
| `theme`        | `theme={{ mode: 'dark', slots: { uploader: { root: '...' } } }}`                         |
...
`theme.mode` supports `light`, `dark`, and `system`. Use `theme.slots` for component classes — the slot map is keyed by component, then by the slot within it.
