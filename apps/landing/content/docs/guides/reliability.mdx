...
These two are genuinely different mechanisms with different storage, different
scope, and — importantly for v3.1.0 — different levels of support.
...
- **Files that were mid-flight come back as `PAUSED`**, not as uploading. The run
  is not resurrected; the user (or your code) decides whether to continue, and
  those files upload again from the beginning.
...
<Callout type="warning" title="Not active in v3.1.0">
...
Plan for full re-uploads of large files after an interruption. If you need true
part-level resume today, use the tus protocol, which handles resumption in the
tus client itself.
