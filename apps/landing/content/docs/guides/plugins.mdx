...
`@useupup/core` has two extension points. A **plugin** is an object that
...
Four cloud-drive plugins, all exported as classes from `@useupup/core`:
...
- **Server mode.** The browser talks only to your server. The drive views route
  through `@useupup/server`'s drive endpoints instead of a plugin — listing,
