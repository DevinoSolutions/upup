...
description: Register @upupjs/server as a Fastify plugin with createUpupPlugin — install, the mount-path option, body parsing, trustProxy origins, and CORS.
---

`@upupjs/server` ships a Fastify adapter: a plugin factory you register on
...
`/multipart/complete`, `/multipart/abort`, `/files/:provider/transfer`)
sends JSON, so the default parser covers all of them.
