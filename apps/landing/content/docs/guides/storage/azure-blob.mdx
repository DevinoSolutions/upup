...
description: Azure Blob Storage has no S3 API, so @upupjs/server rejects it. Upload to Azure from React, Vue, Svelte, or plain JS with upup via client mode and a SAS URL.
---

Azure Blob Storage is the one provider in upup's list that server mode cannot
serve. It has no S3-compatible API, and `@upupjs/server` speaks only S3 — every
...
`@upupjs/core` exports `azure` as a `StorageProvider` value and also lists it in
...
```text
[@upupjs/server] storage.type "azure" has no S3-compatible API and cannot be
