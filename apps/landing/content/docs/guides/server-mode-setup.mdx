...
instead? See [Adapter recipes](#adapter-recipes) below — same config
...
function. Every adapter below wraps that same handler and takes the same
config object — the one you built in step 2. Define it once and share it:
...
// express.json() MUST run before the middleware: the adapter re-serializes
// req.body, so the JSON body parser has to have populated it first.
...
It builds the request URL from `req.originalUrl`, so mounting it under any
prefix works — routes match on path suffix.

### Fastify

```ts
import Fastify from 'fastify'
import { createUpupPlugin } from '@upupjs/server/fastify'
import { upupConfig } from './lib/upup-config'
...
`fastify.all` structurally requires a path. The second argument overrides
it; the default is `/upup/*`. Fastify's built-in JSON parser already
populates `request.body`, so no extra body-parser wiring is needed.
...
Hono is web-native, so `createUpupRoutes` returns the handler unwrapped —
hand it the raw `Request` off the context and return its `Response`.
...
`createUpupNextHandler` returns all four methods and, unlike exporting the
bare handler, corrects the request origin behind a proxy or CDN — pass
`baseUrl` (or `trustProxy: true` to honour `x-forwarded-*`) as a second
argument so OAuth callbacks resolve to your public URL. On Vercel it's a
no-op, since `req.url` is already the public origin. The same export is
also available from `@upupjs/server/next` if you'd rather not add
`@upupjs/next`.
...
<Callout type="warning">
    ...
    the adapter receives a consumed stream and every POST body arrives empty.
    `createUpupPagesHandler` accepts the same `baseUrl` / `trustProxy` options
    as the App Router handler.
</Callout>

### Custom Node server

Any other Node framework can reuse the same bridge the Express, Fastify,
and Pages Router adapters are built on — `toWebRequest` and
`writeWebResponse` from `@upupjs/server/node-bridge`. Don't hand-roll the
conversion; the bridge already handles multi-value headers and skips
`content-length` (Node recomputes it, and a copied value risks a mismatch).
