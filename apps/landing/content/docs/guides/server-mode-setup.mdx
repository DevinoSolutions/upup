...
<Callout type="note">
`getUserId` (or `auth`) is required once you set `providers` or `tokenStore` — it
scopes tokens per user, and without it the upload routes return `403 AUTH_REQUIRED`.
See [Server auth](/docs/guides/server-auth/).
</Callout>
