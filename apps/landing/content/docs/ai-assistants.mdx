...
The fastest path: copy the "Onboard your agent to upup" prompt from
[/agent-setup/](/agent-setup/) (it is also in the docs sidebar and on the
homepage) and paste it into Claude Code, Codex, Cursor, or OpenCode. The agent
fetches `https://useupup.com/agent-setup/prompt.md`, installs the right
`@useupup/*` package, mounts the uploader, adds the presign route, and saves the
context block below into its own instructions file.
