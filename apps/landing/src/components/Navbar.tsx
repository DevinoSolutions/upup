...
// Stable id so the toggle's aria-controls can point at the menu it opens.
...
export default function Navbar() {
    ...
    return (
        // <header> wrapper = the site's banner landmark. The bar was previously a
        // bare <nav>, so the site exposed a navigation landmark but no banner on
        // ANY page. The nav stays `fixed`, so the header itself takes no layout
                ...
                <div className="mx-auto max-w-6xl px-6">
                    <div className="flex items-center justify-between h-24">
                        ...
                        <div className="flex items-center space-x-3">
                            ...
                            {/* Mobile Menu Button. min-h/min-w 11 (44px) meets the
                            WCAG 2.5.8 target size against a 20px icon — the
                            hit area grows, the icon does not. */}
                            <button
                                aria-label="Toggle navigation"
                                aria-expanded={navbarOpen}
                                aria-controls={MOBILE_MENU_ID}
                                className="lg:hidden inline-flex min-h-11 min-w-11 items-center justify-center p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors duration-200"
