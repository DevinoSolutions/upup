...
function HeadlessDemo() {
    const { files, status, progress, addFiles, removeFile, upload, core } =
        useUpupUpload({
    ...
    return (
        ...
        >
            <h3 style={{ margin: '0 0 12px', color: '#30C5F7' }}>
                Headless Hook Demo
            </h3>
            <input
                type="file"
            ...
            <div style={{ fontSize: 13, color: '#aaa', marginBottom: 8 }}>
                Status: {status} | Files: {files.length} | Progress:{' '}
                {progress.percentage}%
            </div>
            {files.map(f => (
                <div
...
function RestrictionsDemo() {
    return (
        <UpupUploader
            ...
            onRestrictionFailed={(file, reason) =>
    ...
    return (
        ...
        >
            <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
                <button onClick={() => setTab('dark')} style={tabStyle('dark')}>
                    Dark Mode
                </button>
                <button
                    onClick={() => setTab('light')}
                    style={tabStyle('light')}
                >
                    Light Mode
                </button>
                <button
                    onClick={() => setTab('headless')}
                    style={tabStyle('headless')}
                >
                    Headless Hook
                </button>
            </div>
