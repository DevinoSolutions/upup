...
test.describe('Adapter switching', () => {
    ...
    test('Camera adapter shows capture UI', async ({ page }) => {
        ...
        await expect(page.locator('[data-upup-slot="camera-uploader"]')).toBeVisible()
    })
    ...
    test('Dropbox adapter shows auth prompt', async ({ page }) => {
        ...
        await expect(page.locator('[data-upup-slot="dropbox-uploader"]')).toBeVisible()
    })
...
test.describe('URL upload', () => {
    test('fetches a file from URL and adds it to the list', async ({ page }) => {
...
test.describe('Multi-file (default limit = 10)', () => {
    ...
    test('each file has its own remove button', async ({ page }) => {
        ...
        await expect(page.locator('[data-testid="upup-file-remove"]').first()).toBeVisible()
    })
    ...
    test('can remove one file while others remain', async ({ page }) => {
        ...
        await expect(page.locator('[data-testid="upup-file-item"]')).toHaveCount(2)
        await page.locator('[data-testid="upup-file-preview"]').first().hover()
        await page.locator('[data-testid="upup-file-remove"]').first().click({ force: true })
        await expect(page.locator('[data-testid="upup-file-item"]')).toHaveCount(1)
    })
})
