...
const MINIO_ENDPOINT = process.env.UPUP_E2E_ENDPOINT ?? 'http://localhost:9000'
...
async function waitFor(url: string, label: string, timeoutMs: number) {
  ...
  while (true) {
    ...
    if (Date.now() - start > timeoutMs) {
      throw new Error(`Timed out (${timeoutMs}ms) waiting for ${label} at ${url}`)
    }
...
export default async function globalSetup() {
  ...
  const harness = spawn('node', ['scripts/upup-e2e-server.mjs'], {
    cwd: REPO_ROOT,
    env: { ...process.env, UPUP_E2E_STORYBOOK_ORIGIN: ALL_STORYBOOK_ORIGINS },
    stdio: 'inherit',
    detached: true,
  })
  harness.unref()
