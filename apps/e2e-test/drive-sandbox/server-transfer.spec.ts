// All four cloud providers proven through the REAL @upupjs/server HTTP surface.
//
...
// ── One real in-process @upupjs/server wired to the live MinIO, shared by both
...
// Boot ONE real in-process @upupjs/server behind a node:http listener via the
...
for (const provider of PROVIDERS) {
    test.describe(`server-mode drive list + transfer through @upupjs/server — ${provider}`, () => {
        test.skip(
            () => !enabled[provider],
