...
test('readTarballEntries extracts members with correct content', () => {
        ...
        makeTgz([
            {
                ...
                content: '{"name":"@x/y","dependencies":{"z":"workspace:*"}}',
            },
    ...
    assert.match(
        entries.get('package/dist/index.js').toString('utf8'),
        /export const a/,
    )
})
...
test('collectExportTargets still flattens a bare-string subpath target', () => {
    const targets = collectExportTargets({
        './styles': './dist/tailwind-prefixed.css',
    })
    assert.deepEqual(new Set(targets), new Set(['dist/tailwind-prefixed.css']))
})
