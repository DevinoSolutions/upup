...
// milestone. Wires @upupjs/server's express adapter to a local MinIO. NOT for
// production (single-user in-memory token store).
//
//   Run: pnpm e2e:minio:server   (loads local-dev/.env.minio via dotenv-cli)
//   Requires: pnpm --filter @upupjs/server build  (resolves @upupjs/server/express)
...
const config = {
    ...
    uploadTokenSecret:
        process.env.UPUP_E2E_UPLOAD_TOKEN_SECRET ??
        'upup-e2e-dev-secret-not-for-prod',
...
app.listen(PORT, () => {
    const gd = config.providers?.googleDrive?.clientId
        ? 'configured'
        : 'NOT configured'
    console.log(`[upup-e2e] presign/transfer server: http://localhost:${PORT}`)
    console.log(
        `[upup-e2e] storage: ${config.storage.endpoint} bucket=${config.storage.bucket}`,
    )
    console.log(`[upup-e2e] CORS origins: ${ALLOWED_ORIGINS.join(', ')}`)
    console.log(
        `[upup-e2e] google-drive OAuth: ${gd} (redirect http://localhost:${PORT}/auth/google-drive/cb)`,
    )
})
