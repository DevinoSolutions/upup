...
const publishable = readdirSync(pkgDir).filter(d => {
    try {
        const pj = JSON.parse(
            readFileSync(join(pkgDir, d, 'package.json'), 'utf8'),
        )
        return (
            pj.private !== true &&
            typeof pj.name === 'string' &&
            pj.name.startsWith('@upup/')
        )
    } catch {
        return false
    }
...
// Single command STRING under shell mode — an args array + shell:true is
// deprecated (Node DEP0190), and Windows needs the shell (pnpm is pnpm.cmd,
// unspawnable directly since the CVE-2024-27980 hardening). Static string,
// no interpolation, so no injection surface.
...
try {
    ...
    console.error(
        'audit-prod: could not parse `pnpm audit --prod --json` output',
    )
...
try {
    allow = new Set(
        (
            JSON.parse(
                readFileSync(
                    join(ROOT, 'scripts/audit-allowlist.json'),
                    'utf8',
                ),
            ).accept ?? []
        ).map(String),
...
for (const n of noted)
    console.log(`note (non-blocking ${n.sev}): ${n.name} [${n.id}]`)
if (blocking.length) {
    console.error(
    ...
    for (const b of blocking)
        console.error(
            `  ${b.sev.toUpperCase()} ${b.name} → upgrade to ${b.patched} [${b.id}]`,
        )
    process.exit(1)
}
console.log(
    `✓ no high/critical advisories in the production trees of ${publishable.length} publishable packages`,
)
