...
export function defineUpupConfig(config: UpupServerConfig): UpupServerConfig {
  ...
  if (!config.storage) {
    ...
    if (hasId || hasSecret) {
      if (!isNonEmpty(config.storage.accessKeyId)) missing.push('storage.accessKeyId')
      if (!isNonEmpty(config.storage.secretAccessKey)) missing.push('storage.secretAccessKey')
    }
  }

  const p = config.providers
  if (p?.googleDrive) {
    if (!isNonEmpty(p.googleDrive.clientId)) missing.push('providers.googleDrive.clientId')
    if (!isNonEmpty(p.googleDrive.clientSecret)) missing.push('providers.googleDrive.clientSecret')
  }
  if (p?.dropbox) {
    if (!isNonEmpty(p.dropbox.appKey)) missing.push('providers.dropbox.appKey')
    if (!isNonEmpty(p.dropbox.appSecret)) missing.push('providers.dropbox.appSecret')
  }
  if (p?.oneDrive) {
    if (!isNonEmpty(p.oneDrive.clientId)) missing.push('providers.oneDrive.clientId')
    if (!isNonEmpty(p.oneDrive.clientSecret)) missing.push('providers.oneDrive.clientSecret')
  }
  if (p?.box) {
    if (!isNonEmpty(p.box.clientId)) missing.push('providers.box.clientId')
    if (!isNonEmpty(p.box.clientSecret)) missing.push('providers.box.clientSecret')
  }
