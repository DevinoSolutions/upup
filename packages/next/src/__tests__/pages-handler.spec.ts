...
describe('createUpupPagesHandler', () => {
  it('bridges a POST body to the core Web Request and maps the JSON response', async () => {
    ...
    expect(new URL(received.req!.url).pathname).toBe('/api/upup-pages/presign')
    expect(await received.req!.json()).toEqual({ name: 'a.png', size: 1, type: 'image/png' })
    expect(res.statusCode).toBe(200)
    expect(Buffer.isBuffer(res.body)).toBe(true)
    expect(JSON.parse((res.body as Buffer).toString())).toEqual({ url: 'https://s3/put' })
  })

  it('does not read a body for GET', async () => {
    const handler = createUpupPagesHandler({} as any)
    const req = mockReq({ method: 'GET', url: '/api/upup-pages/files/google-drive' })
    const res = mockRes()
    await handler(req, res)
  ...
  it('preserves a redirect (3xx + Location) for OAuth', async () => {
    ...
    const handler = createUpupPagesHandler({} as any)
    const res = mockRes()
    await handler(mockReq({ method: 'GET', url: '/api/upup-pages/auth/google-drive' }), res)
  ...
  it('maps a thrown error to a 500 JSON response', async () => {
    ...
    const handler = createUpupPagesHandler({} as any)
    const res = mockRes()
    await handler(mockReq({ method: 'POST', url: '/api/upup-pages/presign', body: '{}' }), res)
  ...
  it('uses an explicit baseUrl for the Web Request origin', async () => {
    const handler = createUpupPagesHandler({} as any, { baseUrl: 'https://app.example.com' })
    await handler(mockReq({ method: 'GET', url: '/api/upup-pages/x' }), mockRes())
