...
describe('resolveOrigin', () => {
  ...
  it('builds the origin from x-forwarded-* when trustProxy is set', () => {
    ...
    expect(resolveOrigin(req, { trustProxy: true })).toBe('https://app.example.com')
  })
...
describe('normalizeRequestOrigin', () => {
  it('rewrites the origin while preserving path, query, method, and body', async () => {
    const req = new Request('https://internal.local/api/upup/presign?x=1', {
      ...
      body: JSON.stringify({ name: 'a.png', size: 10, type: 'image/png' }),
    })
    const out = normalizeRequestOrigin(req, { baseUrl: 'https://app.example.com' })
    ...
    expect(await out.json()).toEqual({ name: 'a.png', size: 10, type: 'image/png' })
  })
