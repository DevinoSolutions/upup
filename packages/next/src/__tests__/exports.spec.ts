...
describe('client export surface (@upup/next)', () => {
  it('re-exports the uploader UI from @upup/react', () => {
...
describe('build output guarantees (run after build; pretest builds)', () => {
  it('emits both entries for esm + cjs + dts + styles', () => {
    for (const f of [
      ...
      'server.js', 'server.cjs', 'server.d.ts',
      'tailwind-prefixed.css',
    ]) {
      expect(existsSync(resolve(distDir, f)), `missing dist/${f}`).toBe(true)
    }
  })

  it("preserves the 'use client' directive in the client entry", () => {
    expect(read('index.js').trimStart().startsWith('"use client"')).toBe(true)
