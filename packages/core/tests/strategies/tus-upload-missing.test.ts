...
describe('TusUpload — optional dep missing', () => {
  it('rejects with an actionable UpupError naming tus-js-client', async () => {
    ...
    try {
      ...
      const { TusUpload } = await import('../../src/strategies/tus-upload')
      const upload = new TusUpload({ protocol: 'tus', endpoint: 'https://example.com/files' } as never)
      const file = new File(['hi'], 'a.txt', { type: 'text/plain' })
      const controller = new AbortController()
      await expect(
        upload.upload(file, {} as never, { onProgress: () => {}, signal: controller.signal }),
