...
const makeImage = (): UploadFile => {
  ...
  return Object.assign(f, { id: 'i1', metadata: {}, thumbnail: null }) as unknown as UploadFile
}
...
describe('thumbnailStep worker branch', () => {
  it('applies the worker thumbnail result (url + rebuilt file)', async () => {
    ...
    const execute = vi.fn(async () => ({ kind: 'thumbnail', thumbnailUrl: 'data:image/jpeg;base64,AAAA', bytes, type: 'image/jpeg', name: 'pic.png.thumbnail.jpg' }))
    const ctx: PipelineContext = { ...ctxBase, worker: { execute } }
    const out = await thumbnailStep().process(makeImage(), ctx)
    expect(execute).toHaveBeenCalledWith(expect.objectContaining({ type: 'thumbnail' }))
