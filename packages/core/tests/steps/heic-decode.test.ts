...
function installFakeLibheif() {
    ...
    vi.doMock('libheif-js/libheif-wasm/libheif-bundle.mjs', () => ({
        default: () => Promise.resolve(libheif),
    }))
...
describe('heicToJpegBlob', () => {
    ...
    it('frees the image handle + context even when display fails (no leak on error)', async () => {
        ...
        const image = {
            get_width: () => 2,
            get_height: () => 2,
            display: (_t: unknown, cb: (r: unknown) => void) => cb(null),
            free,
        }
        class HeifDecoder {
            decoder = { ptr: 1 }
        ...
        vi.doMock('libheif-js/libheif-wasm/libheif-bundle.mjs', () => ({
            default: () => Promise.resolve({ HeifDecoder, heif_context_free }),
        }))
        installCanvas()
        const { heicToJpegBlob } = await import('../../src/steps/heic-decode')

        await expect(
            heicToJpegBlob(new Blob([new Uint8Array([1])]), { quality: 0.9 }),
    ...
    it('returns null (not an error) when no canvas is available', async () => {
        ...
        const { heicToJpegBlob } = await import('../../src/steps/heic-decode')
        const blob = await heicToJpegBlob(new Blob([new Uint8Array([1])]), {
            quality: 0.9,
        })
