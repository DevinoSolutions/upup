...
const makeImage = (): UploadFile => {
  const f = new File([new Uint8Array([1, 2, 3])], 'pic.jpg', { type: 'image/jpeg' })
  return Object.assign(f, { id: 'e1', metadata: { keep: 'me' }, checksumSHA256: 'keep' }) as unknown as UploadFile
}
...
describe('exifStep worker branch', () => {
  it('rebuilds the file from the worker image result, merging metadata', async () => {
    ...
    const execute = vi.fn(async () => ({ kind: 'image', bytes, type: 'image/jpeg', name: 'pic.jpg', metadata: { exifStripped: true, processedSize: 5 } }))
    const ctx: PipelineContext = { ...ctxBase, worker: { execute } }
    const out = await exifStep().process(makeImage(), ctx)
    expect(execute).toHaveBeenCalledWith(expect.objectContaining({ type: 'exif' }))
