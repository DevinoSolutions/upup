...
const makeHeic = (): UploadFile => {
    const f = new File([new Uint8Array([1, 2, 3])], 'photo.heic', {
        type: 'image/heic',
    })
...
describe('heicStep worker branch', () => {
    it('rebuilds the file from the worker image result', async () => {
        ...
        expect(execute).toHaveBeenCalledWith(
            expect.objectContaining({ type: 'heic' }),
        )
    ...
    it('emits pipeline-error (no silent swallow) when conversion throws, still returns original', async () => {
        vi.resetModules()
        vi.doMock('../../src/steps/heic-decode', () => ({
            heicToJpegBlob: vi.fn(async () => {
                throw new Error('boom decode')
            }),
        }))
        ...
        expect(emit).toHaveBeenCalledWith(
            'pipeline-error',
            expect.objectContaining({ scope: 'heic' }),
        )
