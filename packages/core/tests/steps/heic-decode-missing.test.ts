...
describe('heicToJpegBlob — optional dep missing', () => {
    it('throws an actionable UpupError naming libheif-js', async () => {
        vi.doMock('libheif-js/libheif-wasm/libheif-bundle.mjs', () => {
            throw new Error(
                "Cannot find module 'libheif-js/libheif-wasm/libheif-bundle.mjs'",
            )
        })
        ...
        try {
            ...
            vi.resetModules()
            const { heicToJpegBlob } =
                await import('../../src/steps/heic-decode')
            const blob = new Blob([new Uint8Array([0, 0, 0, 1])], {
                type: 'image/heic',
            })
            await expect(heicToJpegBlob(blob)).rejects.toThrow(/libheif-js/)
            await expect(heicToJpegBlob(blob)).rejects.toThrow(
                /npm i libheif-js/,
            )
        } finally {
            if (origDescriptor) {
