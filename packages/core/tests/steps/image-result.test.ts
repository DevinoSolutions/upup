...
const makeFile = (name = 'orig.png', type = 'image/png'): UploadFile => {
  ...
  return Object.assign(f, {
    id: 'id-1', source: 'local', status: 'idle',
    metadata: { keep: 'me' }, url: 'blob:x', relativePath: null,
    key: null, etag: null, fileHash: null, checksumSHA256: 'sum-1', thumbnail: null,
...
describe('uploadFileFromImageResult', () => {
  it('rebuilds an UploadFile from result bytes, preserving identity fields and merging metadata', () => {
    ...
    const out = uploadFileFromImageResult(original, { bytes, type: 'image/jpeg', name: 'orig.jpg', metadata: { compressed: true } })
  ...
  it('falls back to original name/type when result omits them', () => {
    ...
    const out = uploadFileFromImageResult(original, { bytes: new ArrayBuffer(2), type: '' })
