...
describe('serializeCrashRecovery', () => {
  it('serializes a file map into snapshot entries with status', () => {
    const snap = serializeCrashRecovery(new Map([['a', uploadFile('a', 'a.txt')]]), UploadStatus.UPLOADING)
  ...
  it('drops blob: URLs but preserves real URLs', () => {
    const blobFile = uploadFile('b', 'b.txt'); blobFile.url = 'blob:http://x/y'
    const cdnFile = uploadFile('c', 'c.txt'); cdnFile.url = 'https://cdn/c.txt'
    const snap = serializeCrashRecovery(new Map([['b', blobFile], ['c', cdnFile]]), UploadStatus.READY)
    expect((snap.files[0][1] as { url?: string }).url).toBeUndefined()
    expect((snap.files[1][1] as { url?: string }).url).toBe('https://cdn/c.txt')
  })
})
...
describe('reviveCrashRecoverySnapshot', () => {
  ...
  it('round-trips a serialized snapshot', () => {
    const snap = serializeCrashRecovery(new Map([['a', uploadFile('a', 'a.txt')]]), UploadStatus.PAUSED)
  ...
  it('defaults invalid status to IDLE', () => {
    expect(reviveCrashRecoverySnapshot({ files: [], status: 'BOGUS' })!.status).toBe(UploadStatus.IDLE)
  })
})
