...
describe('collectDroppedFiles', () => {
    it('returns empty when dataTransfer has no files or items', async () => {
        const dt = { files: [] as unknown as FileList, items: [] as unknown as DataTransferItemList }
    ...
    it('falls back to dataTransfer.files when no items support entries', async () => {
        ...
        Object.defineProperty(fileList, Symbol.iterator, { value: Array.prototype[Symbol.iterator] })
        const dt = { files: fileList, items: [] as unknown as DataTransferItemList }
        const result = await collectDroppedFiles(dt, false)
