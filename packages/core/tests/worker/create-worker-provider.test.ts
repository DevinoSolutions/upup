...
describe('createWorkerProvider', () => {
  it('returns null when the runtime cannot create a worker', () => {
    expect(createWorkerProvider('code', runtimeWith(null))).toBeNull()
    expect(createWorkerProvider('code', { createWorker: undefined })).toBeNull()
  })

  it('posts the task with id + transfers data, resolves with the matching result', async () => {
    const w = new FakeWorker()
    const p = createWorkerProvider('code', runtimeWith(w))!
  ...
  it('rejects on timeout and stops tracking the request', async () => {
    const w = new FakeWorker()
    const p = createWorkerProvider('code', runtimeWith(w), { timeoutMs: 15 })!
