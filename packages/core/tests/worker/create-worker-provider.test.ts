...
class FakeWorker {
    ...
    respond(res: WorkerResponse) {
        this.onmessage?.({ data: res })
    }
}

const runtimeWith = (w: FakeWorker | null) => ({
    createWorker: () => w as unknown as Worker | null,
})
...
describe('createWorkerProvider', () => {
    ...
    it('posts the task with id + transfers data, resolves with the matching result', async () => {
        ...
        const promise = p.execute<{ kind: string; checksum: string }>({
            type: 'hash',
            data,
        })
        ...
        w.respond({
            id: 1,
            ok: true,
            result: { kind: 'hash', checksum: 'deadbeef' },
        })
    ...
    it('correlates two overlapping calls to the right ids', async () => {
        const w = new FakeWorker()
        const p = createWorkerProvider(runtimeWith(w))!
        const a = p.execute<{ checksum: string }>({
            type: 'hash',
            data: new ArrayBuffer(1),
        })
        const b = p.execute<{ checksum: string }>({
            type: 'hash',
            data: new ArrayBuffer(1),
        })
    ...
    it('rejects on timeout and stops tracking the request', async () => {
        ...
        await expect(
            p.execute({ type: 'hash', data: new ArrayBuffer(1) }),
