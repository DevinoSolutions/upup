...
describe('validateFileRestrictions', () => {
    it('returns [] when no restrictions configured', () => {
        expect(
            validateFileRestrictions(file('a.txt', 'text/plain', 10), {}),
        ).toEqual([])
    })

    it('flags TYPE_MISMATCH with exact message', () => {
        expect(
            validateFileRestrictions(file('a.png', 'image/png', 10), {
                allowedFileTypes: 'image/jpeg',
            }),
        ).toEqual([
            {
                code: UpupErrorCode.TYPE_MISMATCH,
                message: 'File type "image/png" is not accepted',
            },
        ])
    })
    ...
    it('flags FILE_TOO_LARGE with exact message', () => {
        expect(
            validateFileRestrictions(file('a.txt', 'text/plain', 2048), {
                maxFileSize: { size: 1, unit: 'KB' },
            }),
        ).toEqual([
            {
                code: UpupErrorCode.FILE_TOO_LARGE,
                message: 'File "a.txt" exceeds maximum size',
            },
        ])
    })
    ...
    it('flags FILE_TOO_SMALL with exact message', () => {
        expect(
        ...
        ).toEqual([
            {
                ...
                message: 'File "a.txt" is below minimum size',
            },
        ])
    })

    it('collects multiple violations in order [type, max]', () => {
        const errors = validateFileRestrictions(
