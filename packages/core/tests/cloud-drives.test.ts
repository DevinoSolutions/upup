...
describe('CoreOptions.cloudDrives', () => {
    it('accepts and stores the camelCase nested config (all four drives)', () => {
        ...
        expect(core.options.cloudDrives?.googleDrive?.clientId).toBe(
            'gd-client-id',
        )
        expect(core.options.cloudDrives?.oneDrive?.redirectUri).toBe(
            'https://app.example/od_redirect',
        )
...
describe('UpupCore.updateOptions — cloudDrives', () => {
    it('replaces cloudDrives wholesale (plain assign)', () => {
        const core = new UpupCore({
            cloudDrives: {
                googleDrive: { clientId: 'old', apiKey: 'k', appId: 'a' },
            },
        })
        core.updateOptions({
            cloudDrives: {
                googleDrive: { clientId: 'new', apiKey: 'k', appId: 'a' },
            },
        })
