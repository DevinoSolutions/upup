...
import { tokensToVars, tokensToVarRefs, UPUP_VAR_PREFIX } from '../vars'
...
describe('tokensToVars', () => {
    ...
    it('converts camelCase keys to kebab-case', () => {
        ...
        expect(vars['--upup-color-text-muted']).toBe(
            lightPreset.color.textMuted,
        )
        expect(vars['--upup-color-border-active']).toBe(
            lightPreset.color.borderActive,
        )
