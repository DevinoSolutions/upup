...
export class TusUpload implements UploadStrategy {
  ...
  async upload(
    ...
    return new Promise((resolve, reject) => {
      ...
      const chunkSize = this.options.chunkSizeBytes ?? this.options.chunkSize
      if (this.options.headers !== undefined) uploadOptions.headers = this.options.headers
      if (chunkSize !== undefined) uploadOptions.chunkSize = chunkSize
      if (this.options.retryDelays !== undefined) uploadOptions.retryDelays = this.options.retryDelays
      ...
      if (this.options.parallelUploads !== undefined) uploadOptions.parallelUploads = this.options.parallelUploads
