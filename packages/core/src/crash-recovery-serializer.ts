...
function toCrashRecoveryFileSnapshot(
    id: string,
    file: UploadFile,
...
export function reviveCrashRecoverySnapshot(
    ...
    const status = isUploadStatus(snapshot.status)
        ? snapshot.status
        : UploadStatus.IDLE
    const files = snapshot.files
        .map((entry, index): [string, UploadFile] | null => {
            ...
            const id =
                typeof entry[0] === 'string' ? entry[0] : `recovered-${index}`
...
function reviveCrashRecoveryFile(
    id: string,
    value: unknown,
    ...
    const wrappedBlob =
        ...
        value.file instanceof Blob
            ...
            : null
    const props =
        isRecord(value) && wrappedBlob ? value : isRecord(value) ? value : {}
    const nameFromProps =
    ...
    const metadata = isRecord(props.metadata)
        ...
        : {}
    const uploadStatus = isUploadStatus(props.status)
        ? props.status
        : fallbackStatus
    ...
    const url =
        typeof props.url === 'string' && !props.url.startsWith('blob:')
            ? props.url
...
function toRecoverableFile(
    ...
    const lastModified =
        typeof props.lastModified === 'number' ? props.lastModified : Date.now()
