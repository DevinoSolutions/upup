 ...
 */
export async function heicToJpegBlob(input: Blob, options: HeicDecodeOptions = {}): Promise<Blob | null> {
  ...
  try {
    ...
    if (!images || images.length === 0) throw new Error('HEIC: no image found in file')
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      image.display(imageData, (result) => (result ? resolve() : reject(new Error('HEIC: display failed'))))
    })
    ...
    try { libheif.heif_context_free?.(decoder.decoder) } catch { /* already freed */ }
