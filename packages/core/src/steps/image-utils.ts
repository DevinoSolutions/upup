...
function scaleDimensions(width: number, height: number, maxWidthOrHeight?: number): { width: number; height: number } {
...
export async function encodeImageFile(file: UploadFile, options: EncodeImageOptions = {}): Promise<UploadFile | null> {
...
export async function blobToDataUrl(blob: Blob): Promise<string> {
  ...
  const encode = typeof btoa === 'function'
    ? btoa
    : (globalThis as unknown as { Buffer?: { from(input: string, encoding: string): { toString(encoding: string): string } } }).Buffer
      ? (input: string) => (globalThis as unknown as { Buffer: { from(input: string, encoding: string): { toString(encoding: string): string } } }).Buffer.from(input, 'binary').toString('base64')
      : null
  if (!encode) throw new Error('No base64 encoder available')
  return `data:${blob.type || 'application/octet-stream'};base64,${encode(binary)}`
}

export async function createThumbnail(file: UploadFile, options: { width?: number; height?: number; quality?: number; type?: string } = {}): Promise<{ file: File; dataUrl: string; width: number; height: number } | null> {
  const decoded = await decodeImage(file).catch(() => null)
  if (!decoded) return null
  ...
  try {
    ...
    const thumbnailFile = new File([blob], `${file.name}.thumbnail.${ext}`, {
      type: blob.type || type,
      lastModified: file.lastModified,
    })
    return {
...
export function cloneUploadFile(original: UploadFile, replacement: File, metadata: Partial<UploadFileMetadata> = {}): UploadFile {
