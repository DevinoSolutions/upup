      ...
      if (context.worker) {
        try {
          const result = await context.worker.execute<WorkerResult>({
            ...
            params: { mime: file.type, name: file.name, maxWidthOrHeight: _options?.width, quality: _options?.quality },
          })
          if (result.kind === 'thumbnail') {
            file.metadata = { ...file.metadata, thumbnailUrl: result.thumbnailUrl }
            file.thumbnail = { file: new File([result.bytes], result.name, { type: result.type, lastModified: file.lastModified }) }
            return file
          }
