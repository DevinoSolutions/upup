...
describe('createUploaderController', () => {
    ...
    it('config-surface: all four drives register with the camelCase config verbatim', () => {
        ...
        expect(byId.get('google-drive')?.getConfig()).toEqual(
            cloudDrives.googleDrive,
        )
    ...
    it('image-editor commands delegate to the orchestrator', () => {
        const { root } = build()
        const open = vi
            .spyOn(root.orchestrator, 'openImageEditor')
            .mockImplementation(() => {})
        const close = vi
            .spyOn(root.orchestrator, 'closeImageEditor')
            .mockImplementation(() => {})
        const f = { id: 'x' } as never
        root.commands.openImageEditor(f)
    ...
    it('onFilesUploadComplete wrapper invokes connectSSE per completed file', () => {
        ...
        ;(
            root.orchestrator as unknown as {
                callbacks: { onFilesUploadComplete?: (f: unknown[]) => void }
            }
