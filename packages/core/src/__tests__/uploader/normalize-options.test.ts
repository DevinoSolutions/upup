...
describe('normalizeUploaderOptions', () => {
    ...
    it('limit resolution: maxFiles ?? 10', () => {
        ...
        expect(
            normalizeUploaderOptions({ maxFiles: 7 }).coreOptions.limit,
        ).toBe(7)
    ...
    it('mode auto-resolves to server when serverUrl set without uploadEndpoint', () => {
        expect(
            normalizeUploaderOptions({ serverUrl: 'https://s' }).resolved.mode,
        ).toBe('server')
        expect(
        ...
        expect(
            normalizeUploaderOptions({ mode: 'client', serverUrl: 'https://s' })
                .resolved.mode,
        ).toBe('client')
    })
    ...
    it('resolvedImageEditor: true -> enabled defaults', () => {
        expect(
            normalizeUploaderOptions({ imageEditor: true }).resolved
                .imageEditor,
    ...
    it('sources normalize + filter; default when absent', () => {
        expect(
            normalizeUploaderOptions({}).resolved.sources.length,
        ).toBeGreaterThan(0)
        const r = normalizeUploaderOptions({
    ...
    it('i18n: bundle code drives lang/dir; locale string fallback', () => {
        expect(
            normalizeUploaderOptions({ i18n: { locale: 'fr-FR' } }).resolved
                .lang,
        ).toBe('fr-FR')
    ...
    it('accept resolves from the flat allowedFileTypes prop (string or array)', () => {
        expect(
            normalizeUploaderOptions({ allowedFileTypes: 'image/png' }).resolved
                .allowedFileTypes,
        ).toContain('image/png')
        const r = normalizeUploaderOptions({
