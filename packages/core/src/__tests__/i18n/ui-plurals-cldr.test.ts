...
const flat = (b: LocaleBundle) => flattenTranslatorToUiTranslations(createTranslator({ bundle: b }))
...
describe('CLDR category selection (real Arabic cases, synthetic arms — no invented morphology)', () => {
    ...
    it.each([
        ...
        [99, 'MANY'],
        [100, 'OTHER'],
    ])('ar-SA count %i selects %s', (n, exp) => expect(plural(syn, 'x', n as number, 'ar-SA')).toBe(exp))
})

describe('French singular/plural boundary is CLDR-correct (the visible fix)', () => {
    it('count 0 is SINGULAR in French (one = {0,1})', () =>
        expect(render(frFR, 'filesSelected', 0, 'fr-FR')).toBe('0 fichier sélectionné')) // was "0 fichiers sélectionnés"
    it('count 1 singular', () => expect(render(frFR, 'filesSelected', 1, 'fr-FR')).toBe('1 fichier sélectionné'))
    it('count 2 plural', () => expect(render(frFR, 'filesSelected', 2, 'fr-FR')).toBe('2 fichiers sélectionnés'))
})

describe('en-US unchanged; ar-SA real bundle falls back few→other until arms authored', () => {
    it('en-US 1/2/0', () => {
        expect(render(enUS, 'filesSelected', 1, 'en-US')).toBe('1 file selected')
        expect(render(enUS, 'filesSelected', 2, 'en-US')).toBe('2 files selected')
        expect(render(enUS, 'filesSelected', 0, 'en-US')).toBe('0 files selected')
    })
