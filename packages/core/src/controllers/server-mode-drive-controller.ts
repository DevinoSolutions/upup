...
export class ServerModeDriveController implements ObservableController<ServerDriveSnapshot> {
  private snapshot: ServerDriveSnapshot = { state: { status: 'idle' }, folderId: undefined, search: '' }
  ...
  async list(opts?: { folderId?: string; search?: string }): Promise<void> {
    ...
    if (!serverUrl) {
      this.setState({ state: { status: 'error', message: 'Server Mode requires `serverUrl` prop' } })
      return
    }
    this.abort?.abort()
    ...
    try {
      ...
      this.setState({ state: { status: 'error', message: (err as Error).message } })
    } finally {
  ...
  async transfer(file: ServerDriveFile): Promise<ServerDriveTransferResult> {
    ...
    try {
      const res = await fetch(`${serverUrl}/files/${this.deps.provider}/transfer`, {
        method: 'POST',
        credentials: 'include',
  ...
  startAuth(): void {
    ...
    const popup = window.open(`${serverUrl}/auth/${this.deps.provider}`, 'upup-oauth', 'width=600,height=700')
    if (!popup) {
      this.setState({ state: { status: 'error', message: 'Popup blocked. Allow popups and try again.' } })
      return
    }
  ...
  setFolderId(id: string | undefined): void { this.setState({ folderId: id }) }
  setSearch(s: string): void { this.setState({ search: s }) }

  /** Force the reauth view (used by batch consumers on a transfer 401). */
  requestReauth(): void { this.setState({ state: { status: 'reauth' } }) }
