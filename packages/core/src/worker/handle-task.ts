...
async function toImageResult(
    processed: UploadFile | null,
): Promise<Extract<WorkerResult, { kind: 'image' }> | null> {
    ...
    try {
        ...
        switch (req.type) {
            ...
            case 'compress': {
                ...
                if (
                    processed.size >= req.data.byteLength &&
                    !maxBytes &&
            ...
            default:
                return {
                    id: req.id,
                    ok: false,
                    error: `unknown task type: ${(req as { type: string }).type}`,
                }
        }
    } catch (e) {
