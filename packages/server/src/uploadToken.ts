...
export async function verifyUploadToken(
  ...
  if (typeof token !== 'string' || !token.includes('.')) {
    throw new UploadTokenError('malformed', 'Upload token is missing or malformed')
  }
  ...
  if (!timingSafeEqual(sig, expected)) {
    throw new UploadTokenError('bad_signature', 'Upload token signature is invalid')
  }
  let payload: UploadTokenPayload
  try {
    payload = JSON.parse(new TextDecoder().decode(base64UrlToBytes(body)))
  } catch {
    throw new UploadTokenError('malformed', 'Upload token payload is not valid JSON')
  }
  if (
