...
describe("validateReturnTo", () => {
  ...
  it("rejects cross-origin not in the allowlist", () => {
    const result = validateReturnTo("https://evil.example/x", req, {
  ...
  it("allows cross-origin when cors.allowedOrigins includes the origin", () => {
    const result = validateReturnTo("https://app.example/done", req, {
  ...
  it("returns undefined for garbage input (javascript: scheme has null origin, not in allowlist)", () => {
    ...
    expect(
      validateReturnTo("javascript:alert(1)", req, undefined),
    ).toBeUndefined();
  });
  ...
  it("rejects userinfo confusion (real host is evil.example, not allowlisted app.example)", () => {
    const result = validateReturnTo("https://app.example@evil.example/x", req, {
      allowedOrigins: ["https://app.example"],
    });
    expect(result).toBeUndefined();
  });
...
describe("concreteAllowedOrigins", () => {
  it("filters out wildcard entries", () => {
    expect(
      concreteAllowedOrigins({ allowedOrigins: ["*", "https://app.example"] }),
