...
import { nodeHeadersToWeb, toWebRequest, writeWebResponse, type NodeResponseSink } from '../src/node-http-bridge'
...
describe('nodeHeadersToWeb', () => {
  it('converts a flat Record<string,string> to Headers', () => {
    const headers = nodeHeadersToWeb({ 'content-type': 'application/json', 'x-custom': 'abc' })
  ...
  it('skips undefined header values', () => {
    const headers = nodeHeadersToWeb({ 'x-present': 'yes', 'x-absent': undefined })
...
describe('toWebRequest — table: express-style / fastify-style / next-style bodies bridge identically', () => {
  const table: Array<{ label: string; body: RequestInit['body'] }> = [
    { label: 'express-style (JSON.stringify(req.body))', body: JSON.stringify({ hello: 'world' }) },
    { label: 'fastify-style (same shape as express)', body: JSON.stringify({ a: 1 }) },
    { label: 'next-style (raw Buffer)', body: Buffer.from(JSON.stringify({ raw: true })) },
  ]
  ...
  it('sends no body for HEAD requests', async () => {
    const req = toWebRequest({ url: 'http://localhost:3000/x', method: 'HEAD', headers: {} })
...
describe('writeWebResponse', () => {
  ...
  it('copies response headers EXCEPT content-length (Node recomputes it -- a stale copy risks a mismatch)', async () => {
    ...
    const webRes = new Response('hello world', {
      status: 200,
      headers: { 'content-type': 'text/plain', 'content-length': '999', 'x-request-id': 'abc' },
    })
    await writeWebResponse(sink, webRes)
    const setHeaderCalls = sink.setHeader.mock.calls.map((c) => c[0].toLowerCase())
