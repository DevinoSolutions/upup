...
describe("token helpers", () => {
  ...
  it("scopes by userId and provider independently", async () => {
    ...
    expect((await getTokens(store, "u1", "google-drive"))?.accessToken).toBe(
      "a1",
    );
    expect((await getTokens(store, "u2", "google-drive"))?.accessToken).toBe(
      "a2",
    );
...
describe("GET /auth/:provider/cb", () => {
  ...
  it("propagates provider errors", async () => {
    const handler = createUpupHandler(baseConfig());
    const res = await handler(
      new Request("http://localhost/auth/google-drive/cb?error=access_denied", {
        method: "GET",
      }),
    );
    expect(res.status).toBe(400);
