...
describe('token helpers', () => {
  ...
  it('scopes by userId and provider independently', async () => {
    ...
    expect((await getTokens(store, 'u1', 'google-drive'))?.accessToken).toBe('a1')
    expect((await getTokens(store, 'u2', 'google-drive'))?.accessToken).toBe('a2')
    expect((await getTokens(store, 'u1', 'dropbox'))?.accessToken).toBe('d1')
  })
})
...
describe('OAuth state', () => {
  ...
  it('consumeOAuthState deletes the entry after reading', async () => {
    const store = new InMemoryTokenStore()
    await saveOAuthState(store, 'abc', { userId: 'u', provider: 'google-drive' })
...
describe('GET /files/:provider', () => {
  ...
  it('drops stale token and returns reauth on upstream 401', async () => {
    const store = new InMemoryTokenStore()
    await setTokens(store, DEFAULT_USER_ID, 'dropbox', { accessToken: 'stale' })
    fetchSpy = vi
