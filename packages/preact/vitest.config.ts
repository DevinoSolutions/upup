...
// Point @useupup/react to our PREACT-compiled dist (already has react→preact/compat applied by tsup)
const preactDist = resolve(__dirname, 'dist/index.js')

// NOTE: run tests via `pnpm test` (triggers pretest→tsup), not bare `vitest` —
// the suite resolves @useupup/react to the COMPILED dist, which pretest rebuilds.
...
export default defineConfig({
    resolve: {
        ...
        dedupe: ['preact'],
        alias: [
            // Map @useupup/react to OUR preact-compiled dist instead of react source
            { find: '@useupup/react', replacement: preactDist },
