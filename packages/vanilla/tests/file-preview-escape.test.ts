...
describe('vanilla file-preview-portal — Escape closes (F-605)', () => {
  it('closes the portal on a window-level Escape keydown', async () => {
    ...
    const previewBtn = Array.from(host.querySelectorAll('button'))
      .find((b) => b.textContent?.includes('Click to preview')) as HTMLButtonElement | undefined
    expect(previewBtn, 'preview trigger button present').toBeTruthy()
    previewBtn!.click()
    await Promise.resolve()

    expect(host.querySelector('[data-upup-slot="file-preview-portal"]'), 'portal open after click').toBeTruthy()

    // Dispatch on window, not the dialog — focus never entered the dialog.
    window.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape' }))
    await Promise.resolve()

    expect(host.querySelector('[data-upup-slot="file-preview-portal"]'), 'portal closed after Escape').toBeNull()
    up.destroy()
  })
  ...
  it('removes its window listener when the portal closes (no leak across files)', async () => {
    ...
    const previewBtns = () => Array.from(host.querySelectorAll('button'))
      .filter((b) => b.textContent?.includes('Click to preview')) as HTMLButtonElement[]

    // Open + close the first file's portal via the X button (not Escape).
    previewBtns()[0].click()
    await Promise.resolve()
    const closeBtn = host.querySelector('[data-upup-slot="file-preview-portal"] button[aria-label]') as HTMLButtonElement
    closeBtn.click()
    await Promise.resolve()
    expect(host.querySelector('[data-upup-slot="file-preview-portal"]')).toBeNull()
