...
export function filePreview(
  ...
  return html`
    <div
      ...
      tabindex="0"
      @click=${onFileClick}
      @keydown=${(e: KeyboardEvent) => { if (e.key === 'Enter' || e.key === ' ') onFileClick() }}
    >
      <div
        class=${cn(
          ...
          {
            [slot.fileThumbnailMultiple ?? '']: !!slot.fileThumbnailMultiple && filesSize > 1,
            [slot.fileThumbnailSingle ?? '']: !!slot.fileThumbnailSingle && filesSize === 1,
          },
          ...
          ? html`
            <button
              ...
              class=${cn(
                ...
                { 'upup-text-[#4A9EFF] hover:upup-text-blue-300': isDark },
