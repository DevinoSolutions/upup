...
function filterItems(item: DriveFile, accept: string): boolean {
  ...
  return accept.split(',').some((pattern) => {
    ...
    if (p.endsWith('/*')) return item.mimeType?.startsWith(p.replace('/*', '/')) ?? false
...
export function driveBrowser(ctx: UploaderContext, props: DriveBrowserProps) {
  ...
  const displayedItems = searchDriveFiles(items as DriveFile[], ss.searchTerm) ?? []
  ...
  const inner = html`
    <div data-testid="upup-drive-browser" class="upup-grid upup-h-full upup-w-full upup-grid-rows-[auto,1fr,auto] upup-overflow-auto">
      ${driveBrowserHeader(ctx, {
        ...
        onSearch: (v: string) => { ss.searchTerm = v; ctx.invalidate() },
      })}
      ...
      ${path.length > 0 ? html`
        <div
          class=${cn(
            ...
            {
              'upup-bg-white/10 upup-text-[#fafafa] dark:upup-bg-white/10 dark:upup-text-[#fafafa]': isDark,
            },
            slot.driveBody,
          )}
        >
          ...
          ${displayedItems.length > 0 ? html`
            <ul class="upup-p-2">
              ${repeat(
                ...
                (file) => driveBrowserItem(ctx, {
                  item: file as DriveFile,
                  isSelected: selectedFiles.some((f) => f.id === (file as DriveFile).id),
                  isClickLoading: isClickLoading || showLoader,
                  onClick: () => { if (!isClickLoading && !showLoader) handleClick(file as DriveFile) },
                }),
              )}
            </ul>` : nothing}
      ...
      ${selectedFiles.length > 0 || !!onSelectCurrentFolder ? html`
        ...
        >
          ...
          <button
            ...
            @click=${() => { handleSubmit() }}
          >
            ${t(plural(tr, 'addFiles', selectedFiles.length), { count: selectedFiles.length })}
          </button>
          <button
            class=${cn(
