...
export function driveBrowserHeader(
  ...
  const { user, path, setPath, handleSignOut, onSearch, searchTerm, showSearch } = args
            ...
            ${repeat(
              ...
              (p, i) => html`
                <p
                  ...
                  style="max-width: ${100 / path.length}%; pointer-events: ${i === path.length - 1 ? 'none' : 'auto'}"
                  @click=${() => setPath(path.slice(0, i + 1))}
                >
                  <span class="upup-group-hover:upup-underline upup-truncate">${p.name}</span>
                  ${i !== path.length - 1 ? html` &gt; ` : nothing}
                </p>`,
            )}
          </div>` : nothing}
        ...
        >
          <input
            ...
            class=${cn(
              ...
              {
                'upup-bg-white/5 upup-text-[#6D6D6D] dark:upup-bg-white/5 dark:upup-text-[#6D6D6D]': isDark,
              },
