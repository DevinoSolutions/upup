...
export function driveAuthFallback(
  ...
  const inner = html`
    <div class="upup-flex upup-h-full upup-w-full upup-flex-col upup-items-center upup-justify-center upup-gap-4 upup-p-6 upup-text-center">
      <p class=${cn('upup-text-sm upup-text-[#333]', { 'upup-text-[#FAFAFA] dark:upup-text-[#FAFAFA]': isDark }, slot.adapterView)}>${t(tr.authenticatePrompt, { provider: opts.providerName })}</p>
  ...
  return adapterViewContainer(ctx, { dataUpupSlot: opts.dataUpupSlot ?? 'drive-auth-fallback' }, inner)
}
