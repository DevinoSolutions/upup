...
export function App(ctx: RootContext) {
  ...
  return html`
    ...
    >
      ...
      >
        <section
          ...
          class=${cn(
            ...
            {
              [slotOverrides.containerFull ?? '']: !!slotOverrides.containerFull && !ctx.props.mini,
              [slotOverrides.containerMini ?? '']: !!slotOverrides.containerMini && ctx.props.mini,
            },
          )}
        >
          ${mainBox(ctx)}
          ...
          ${!ctx.props.mini && ctx.props.showBranding
            ? html`
                ...
                >
                  <a href="https://useupup.com/" target="_blank" rel="noopener noreferrer" class="upup-flex upup-items-center upup-gap-[5px]">
                    ${isDark
                      ? html`<img src=${logoDark} width="61" height="13" alt="logo-dark" />`
                      : html`<img src=${logoLight} width="61" height="13" alt="logo-light" />`}
                  </a>
                  <a href="https://devino.ca/" target="_blank" rel="noopener noreferrer" class="upup-flex upup-flex-row upup-items-center upup-justify-end upup-gap-1">
                    <span
                      class=${cn('upup-mr-0.5 upup-text-xs upup-leading-5 upup-text-[#6D6D6D] md:upup-text-sm', {
                        'upup-text-gray-300 dark:upup-text-gray-300': isDark,
                      })}
                    >${ctx.translations.builtBy}</span>
                    ${isDark
                      ? html`<img src=${devinoDark} width="61" height="13" alt="logo-dark" />`
                      : html`<img src=${devinoLight} width="61" height="13" alt="logo-light" />`}
                  </a>
                </div>
              `
            : nothing}
      ...
      <input
        ${ref((el) => ctx.registerFileInput((el as HTMLInputElement) ?? null))}
        type="file"
...
export function createRenderLoop(ctx: RootContext, rootEl: HTMLElement) {
  ...
  const renderApp = () => {
    ...
    if (scope) scope.classList.toggle('dark', ctx.theme.getSnapshot().isDark)
  }
