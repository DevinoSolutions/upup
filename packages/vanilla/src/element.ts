...
export class UpupUploaderElement extends HTMLElement {
  ...
  get instance(): UpupInstance | null { return this._instance }

  set config(value: CreateUploaderOptions) {
    this._config = value ?? {}
    if (this.isConnected) this.remount()
  }
  get config(): CreateUploaderOptions { return this._config }
    ...
    for (const [coreEvent, domEvent] of FORWARDED_EVENTS) {
      this.forwardUnsubs.push(
        this._instance.on(coreEvent, (...args: unknown[]) => {
          this.dispatchEvent(new CustomEvent(domEvent, { detail: args.length === 1 ? args[0] : args, bubbles: true, composed: true }))
        }),
      )
    }
  }
  ...
  private remount() { this.unmount(); this.mount() }
}
