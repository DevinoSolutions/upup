  ...
  if (!el) {
    throw new TypeError(
      `@upup/vanilla: createUploader() target not found: ${
        typeof target === 'string' ? `selector "${target}"` : 'the provided element is null'
      }.`,
    )
  }
  return el
...
export function createHiddenFileInput(accept: string, multiple: boolean): HTMLInputElement {
