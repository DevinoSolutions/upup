...
export default tseslint.config(
  ...
  comments.recommended,
  {
    ...
    rules: {
      ...
      '@typescript-eslint/ban-ts-comment': [
        'error',
        { 'ts-expect-error': { descriptionFormat: '^: .+$' }, 'ts-ignore': true, 'ts-nocheck': true },
      ],
      ...
      'no-restricted-syntax': [
        ...
        {
          selector: "CallExpression[callee.property.name='emit'][arguments.0.value='error']",
          message:
        ...
        {
          selector: "MethodDefinition[key.name=/^(dispose|teardown)$/]",
          message: 'Lifecycle verb is destroy() everywhere — dispose/teardown were retired in N3.',
        },
        {
          selector:
        ...
        },
        {
          ...
          message: 'onFileRemove alias is dead (N3) — the callback is onFileRemoved (past tense).',
        },
        // N4: DOM contract is the source vocabulary
        {
          selector: 'Literal[value=/upup-adapter-|adapter-view|adapter-click|data-upup-slot=\\"adapter/]',
          message:
      ...
      'no-restricted-imports': [
        'error',
        {
          ...
          paths: [
            { name: 'libheif-js', message: 'Heavy dep — dynamic import() behind @upup/core/steps/heic only (principle 6).' },
            {
              name: 'tus-js-client',
              message: 'Heavy dep — dynamic import() behind @upup/core/strategies/tus-upload only (principle 6).',
            },
          ],
        },
      ],
  ...
  },
  {
    ...
    files: ['**/*.test.{ts,tsx}', '**/*.spec.{ts,tsx}', '**/tests/**', '**/__tests__/**'],
    ...tseslint.configs.disableTypeChecked,
    rules: {
    ...
    rules: {
      ...
      'no-restricted-imports': [
        'error',
        {
          patterns: [
            {
              group: ['react', 'react-*', 'vue', 'svelte', 'svelte/*', '@angular/*', 'preact', 'preact/*'],
              message: '@upup/core has ZERO framework dependencies — keep it that way.',
            },
            {
              group: ['@upup/core', '@upup/core/*'],
              message: 'Do not import the public barrel from inside core (cycle risk) — use relative imports.',
            },
            { group: ['@upup/*/src/*', '**/packages/*/src/**'], message: 'Packages consume dist/, never src/ (principle 5).' },
          ],
          paths: [
            { name: 'libheif-js', message: 'Dynamic import() behind steps/heic only (principle 6).' },
            { name: 'tus-js-client', message: 'Dynamic import() behind strategies/tus-upload only (principle 6).' },
          ],
        },
      ],
    },
