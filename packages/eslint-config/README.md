Shared ESLint flat config for the publishable `@useupup/*` packages. Import it from
a package's `eslint.config.mjs` (`import config from '@useupup/eslint-config';
...
The react-family packages (`@useupup/react`, `@useupup/preact`) additionally compose
