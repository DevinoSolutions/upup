...
export interface UpupUploadHandle {
  ...
  progress: WritableSignal<{ totalFiles: number; completedFiles: number; percentage: number }>
...
export function createUpupUpload(options: UseUpupUploadOptions): UpupUploadHandle {
  ...
  return {
    ...
    start() {
      ...
      unsubs.push(core.on('state-change', () => {
      ...
      if (options.onFileAdded) {
        unsubs.push(core.on('files-added', (payload) => {
          options.onFileAdded!(payload as UploadFile[])
        }))
      }
      if (options.onFileRemoved) {
        unsubs.push(core.on('file-removed', (payload) => {
          options.onFileRemoved!(payload as UploadFile)
        }))
      }
      if (options.onUploadProgress) {
        unsubs.push(core.on('upload-progress', (payload) => {
          options.onUploadProgress!(payload as { fileId: string; loaded: number; total: number })
        }))
      }
      if (options.onUploadComplete) {
        unsubs.push(core.on('upload-all-complete', (payload) => {
