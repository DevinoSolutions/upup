...
describe('UpupUploaderComponent', () => {
    ...
    it('forwards the 5 core events to the matching outputs', () => {
        ...
        f.componentInstance.fileRemoved.subscribe(
            v => (seen['fileRemoved'] = v),
        )
        f.componentInstance.uploadProgress.subscribe(
            v => (seen['uploadProgress'] = v),
        )
        f.componentInstance.uploadAllComplete.subscribe(
            v => (seen['uploadAllComplete'] = v),
        )
        ...
        expect(seen['uploadProgress']).toEqual({
            fileId: 'a',
            loaded: 1,
            total: 2,
        })
    ...
    it('onInputChange with files calls store.handleSetSelectedFiles', () => {
        ...
        const handleSetSpy = vi
            .spyOn(f.componentInstance.store, 'handleSetSelectedFiles')
