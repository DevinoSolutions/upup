...
describe('LoadGapiService', () => {
    ...
    it('load() calls loadGoogleIdentityServices', async () => {
        const spy = vi.spyOn(await import('@upup/core'), 'loadGoogleIdentityServices').mockResolvedValue(undefined as any)
        TestBed.configureTestingModule({ providers: [LoadGapiService] })
        const svc = TestBed.inject(LoadGapiService)
        svc.load()
...
describe('OneDriveService delegation', () => {
    ...
    it('init() delegates to controller.init()', () => { svc.init(); expect(ctrl.init).toHaveBeenCalled() })
    it('signIn() delegates to controller.signIn()', () => { svc.signIn(); expect(ctrl.signIn).toHaveBeenCalled() })
    it('signOut() delegates to controller.signOut()', () => { svc.signOut(); expect(ctrl.signOut).toHaveBeenCalled() })
    it('setPath() delegates to controller.setPath()', () => {
        const p = [] as any; svc.setPath(p); expect(ctrl.setPath).toHaveBeenCalledWith(p)
    })
    it('handleClick() delegates to controller.handleClick()', () => {
        const f = { id: 'f1' } as any; svc.handleClick(f); expect(ctrl.handleClick).toHaveBeenCalledWith(f)
    })
    it('handleCancelDownload() delegates', () => { svc.handleCancelDownload(); expect(ctrl.handleCancelDownload).toHaveBeenCalled() })
...
describe('DropboxService delegation', () => {
    ...
    it('authenticate() delegates to controller.signIn()', () => { svc.authenticate(); expect(ctrl.signIn).toHaveBeenCalled() })
    it('logout() delegates to controller.signOut()', () => { svc.logout(); expect(ctrl.signOut).toHaveBeenCalled() })
    it('handleClick() delegates to controller.handleClick()', () => {
        const f = { id: 'f2' } as any; svc.handleClick(f); expect(ctrl.handleClick).toHaveBeenCalledWith(f)
    })
    it('setPath() delegates to controller.setPath()', () => {
        const p = [] as any; svc.setPath(p); expect(ctrl.setPath).toHaveBeenCalledWith(p)
    })
...
describe('BoxService delegation', () => {
    ...
    it('handleClick() delegates', () => {
        const f = { id: 'f3' } as any; svc.handleClick(f); expect(ctrl.handleClick).toHaveBeenCalledWith(f)
    })
