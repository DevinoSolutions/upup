...
export class ServerModeDriveService {
    ...
    private bridge!: SignalStore<import('@upup/core').ServerDriveSnapshot>
    ...
    startAuth(): void { this.controller.startAuth() }
    setFolderId(id: string | undefined): void { this.controller.setFolderId(id) }
    setSearch(s: string): void { this.controller.setSearch(s) }
    ...
    async transfer(): Promise<void> {
        ...
        try {
            for (const file of toTransfer) {
                ...
                if (r.status === 'reauth') { this.controller.requestReauth(); return }
                if (r.status === 'error') { this.controller.setError(r.message); return }
