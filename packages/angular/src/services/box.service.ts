...
/**
 * Angular port of useBox.ts (svelte composable).
 *
 * Pure delegation to AdapterBrowserController with BOX_DESCRIPTOR (popup auth).
...
export class BoxService {
    ...
    readonly boxFiles = computed(() => this.state().folder)
    ...
    // ── Forwarding methods ────────────────────────────────────────
    authenticate(): void { this.controller.signIn() }
    logout(): void { this.controller.signOut() }
