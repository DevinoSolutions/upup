...
export class FileItemComponent {
    ...
    get containerClass(): string {
        ...
        return cn(
            ...
            {
                [slotClasses.fileItemMultiple ?? '']: !!slotClasses.fileItemMultiple && filesSize > 1,
                [slotClasses.fileItemSingle ?? '']: !!slotClasses.fileItemSingle && filesSize === 1,
            },
        )
    }
    ...
    openPreviewPortal(): void {
        ...
        this.store.core?.emit('file-preview-open', { fileId: this.file.id, fileName: this.file.name })
    }

    closePreviewPortal(): void {
        this.showPreviewPortal = false
        this.store.core?.emit('file-preview-close', { fileId: this.file.id, fileName: this.file.name })
    }
}
