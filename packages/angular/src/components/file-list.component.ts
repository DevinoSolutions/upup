                ...
                @if (shouldVirtualize && virtualizer) {
                    ...
                    >
                        @for (vi of virtualItems; track vi.key) {
                            ...
                            >
                                <upup-file-item [file]="sortedFiles[vi.index]" />
                            </div>
                        }
                    </div>
                } @else {
            ...
            <div [class]="footerClass">
                ...
                >
                    <button
                        ...
                        (click)="onUploadClick()"
                        [disabled]="isUploadActive(store.uploadStatus()) || store.uploadStatus() === UploadStatus.PAUSED || store.uiProps.isProcessing"
                    >
                        {{ uploadButtonText }}
                    </button>
                </upup-should-render>
                    ...
                    <div class="upup-flex upup-items-center upup-gap-2">
                        ...
                        >
                            <button
                                ...
                                (click)="store.uploadStatus() === UploadStatus.PAUSED ? store.handleResume() : store.handlePause()"
                                [attr.aria-label]="store.uploadStatus() === UploadStatus.PAUSED ? store.translations().resumeUpload : store.translations().pauseUpload"
                                [attr.title]="store.uploadStatus() === UploadStatus.PAUSED ? store.translations().resumeUpload : store.translations().pauseUpload"
                            >
                                @if (store.uploadStatus() === UploadStatus.PAUSED) {
                                    <upup-player-play-filled-icon [size]="14" />
                                } @else {
                                    <upup-player-pause-filled-icon [size]="14" />
                                }
                            </button>
...
export class FileListComponent implements AfterViewInit, OnDestroy {
    ...
    get shouldVirtualize(): boolean {
        return this.sortedFiles.length >= VIRTUAL_SCROLL_THRESHOLD && this.store.viewMode() !== 'grid'
    }
    ...
    get innerListClass(): string {
        return cn(
            this.store.uiProps.isProcessing ? 'upup-pointer-events-none upup-opacity-75' : '',
    ...
    get standardInnerClass(): string {
        ...
        return cn(
            ...
            },
            {
                ...
                [slotClasses.fileListContainerInnerSingle ?? '']: !!slotClasses.fileListContainerInnerSingle && sortedFiles.length === 1,
            },
        )
    }
    ...
    get showProgressText(): boolean {
        return isUploadActive(this.store.uploadStatus()) || this.store.uploadStatus() === UploadStatus.PAUSED
    }

    get progressTextClass(): string {
        const dark = this.store.isDark()
        return cn(
            'upup-text-[11px] upup-text-gray-500',
