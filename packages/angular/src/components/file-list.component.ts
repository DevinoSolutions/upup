  ...
  template: `
    ...
    >
        ...
        @if (shouldVirtualize && virtualizer) {
          ...
          >
            @for (vi of virtualItems; track vi.key) {
              <div [attr.data-index]="vi.index" [style]="virtualRowStyle(vi)">
      ...
      <div [class]="footerClass">
        ...
        @if (
          store.uploadStatus() === UploadStatus.FAILED && store.uploadError()
        ) {
          <p
        ...
        @if (store.uploadStatus() === UploadStatus.SUCCESSFUL) {
          <button [class]="doneButtonClass" (click)="store.handleDone()">
...
export class FileListComponent implements AfterViewInit, OnDestroy {
  ...
  ngDoCheck(): void {
    ...
    const sv =
      count >= VIRTUAL_SCROLL_THRESHOLD && this.store.viewMode() !== "grid";
