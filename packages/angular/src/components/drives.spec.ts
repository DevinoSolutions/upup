...
function makeFakeGoogleDriveService(state: ReturnType<typeof signal<Partial<AdapterBrowserState>>>) {
    ...
    class FakeGoogleDriveService {
        ...
        readonly isClickLoading = computed(() => state().isClickLoading ?? false)
        init = vi.fn()
        destroy = vi.fn()
        retryAuth = vi.fn()
        handleSignOut = vi.fn()
...
function makeFakeDropboxService(state: ReturnType<typeof signal<Partial<AdapterBrowserState>>>) {
    ...
    class FakeDropboxService {
        readonly isAuthenticated = computed(() => state().isAuthenticated ?? false)
        readonly isLoading = computed(() => state().isLoading ?? true)
        readonly token = computed(() => state().isAuthenticated ? ('active' as unknown as ReturnType<DropboxService['token']>) : undefined)
        readonly user = computed(() => state().user)
...
function makeFakeBoxService(state: ReturnType<typeof signal<Partial<AdapterBrowserState>>>) {
    ...
    class FakeBoxService {
        readonly isAuthenticated = computed(() => state().isAuthenticated ?? false)
        readonly isLoading = computed(() => state().isLoading ?? true)
        readonly token = computed(() => state().isAuthenticated ? ('active' as unknown as ReturnType<BoxService['token']>) : undefined)
        readonly user = computed(() => state().user)
...
function makeFakeOneDriveService(state: ReturnType<typeof signal<Partial<AdapterBrowserState>>>) {
    ...
    class FakeOneDriveService {
        readonly isAuthenticated = computed(() => state().isAuthenticated ?? false)
        readonly isLoading = computed(() => state().isLoading ?? true)
        readonly token = computed(() => state().isAuthenticated ? ('active' as unknown as ReturnType<OneDriveService['token']>) : undefined)
        readonly user = computed(() => state().user)
...
describe('DriveAuthFallbackComponent', () => {
    ...
    it('resolves {{provider}} in "Google Drive" — no literal placeholder in DOM', () => {
        ...
        fixture.componentInstance.onRetry = () => { /* noop */ }
...
describe('DriveBrowserItemComponent', () => {
    ...
    it('renders file name', () => {
        const file = { id: 'f1', name: 'report.pdf', isFolder: false, mimeType: 'application/pdf' } as any
    ...
    it('has data-upup-slot="drive-browser-item"', () => {
        const file = { id: 'f1', name: 'a.txt', isFolder: false, mimeType: 'text/plain' } as any
        const fixture = mountItem(file)
        expect(fixture.nativeElement.querySelector('[data-upup-slot="drive-browser-item"]')).not.toBeNull()
    })

    it('calls handleClick with the file on click', () => {
        const file = { id: 'f1', name: 'doc.pdf', isFolder: false, mimeType: 'application/pdf' } as any
        const fixture = mountItem(file)
        const handleClick = fixture.componentInstance.handleClick as ReturnType<typeof vi.fn>

        const el = fixture.nativeElement.querySelector('[data-upup-slot="drive-browser-item"]') as HTMLElement
        el.click()
...
describe('DriveBrowserComponent', () => {
    ...
    it('renders data-testid="upup-drive-browser" when driveFiles is set', () => {
        ...
        expect(fixture.nativeElement.querySelector('[data-testid="upup-drive-browser"]')).not.toBeNull()
    })

    it('renders N DriveBrowserItem elements for N files', () => {
        const items = [
            { id: 'f1', name: 'a.txt', isFolder: false, mimeType: 'text/plain' },
            { id: 'f2', name: 'b.pdf', isFolder: false, mimeType: 'application/pdf' },
            { id: 'f3', name: 'c.jpg', isFolder: false, mimeType: 'image/jpeg' },
        ] as DriveFile[]
        const fixture = mountWithItems(items)

        const rendered = fixture.nativeElement.querySelectorAll('[data-upup-slot="drive-browser-item"]')
    ...
    it('clicking a file calls handleClick with the file', () => {
        const file = { id: 'f1', name: 'doc.pdf', isFolder: false, mimeType: 'application/pdf' } as DriveFile
        const fixture = mountWithItems([file])
        const comp = fixture.componentInstance

        const itemEl = fixture.nativeElement.querySelector('[data-upup-slot="drive-browser-item"]') as HTMLElement
        itemEl.click()
        expect(comp.handleClick).toHaveBeenCalledWith(expect.objectContaining({ id: 'f1' }))
    })
})
...
describe('ClientGoogleDriveUploaderComponent', () => {
    ...
    function mountWithState(statePartial: Partial<AdapterBrowserState>) {
        ...
        TestBed.overrideComponent(ClientGoogleDriveUploaderComponent, {
            set: { providers: [{ provide: GoogleDriveService, useClass: FakeService }] },
        })
        const fixture = TestBed.createComponent(ClientGoogleDriveUploaderComponent)
        fixture.detectChanges()
        return { fixture, stateSig }
    }

    it('renders DriveAuthFallback (no upup-drive-browser) when not authed and authCancelled=true', () => {
        const { fixture } = mountWithState({ isAuthReady: true, authCancelled: true, token: undefined })
        expect(fixture.nativeElement.querySelector('[data-testid="upup-drive-browser"]')).toBeNull()
    ...
    it('renders DriveAuthFallback when isAuthReady=true and no token', () => {
        const { fixture } = mountWithState({ isAuthReady: true, authCancelled: false, token: undefined })
        expect(fixture.nativeElement.querySelector('[data-testid="upup-drive-browser"]')).toBeNull()
    })

    it('renders upup-drive-browser when token is present', () => {
        const folder = { id: 'root', name: 'Drive', children: [], isFolder: true } as any
...
describe('ClientDropboxUploaderComponent', () => {
    ...
    function mountWithState(statePartial: Partial<AdapterBrowserState>) {
        ...
        TestBed.overrideComponent(ClientDropboxUploaderComponent, {
            set: { providers: [{ provide: DropboxService, useClass: FakeService }] },
        })
    ...
    it('renders auth fallback with "Dropbox" when not authenticated', () => {
        const fixture = mountWithState({ isAuthenticated: false, isLoading: false })
        expect(fixture.nativeElement.querySelector('[data-testid="upup-drive-browser"]')).toBeNull()
    ...
    it('renders upup-drive-browser when authenticated', () => {
        const folder = { id: 'root', name: 'Dropbox', children: [], isFolder: true } as any
        const fixture = mountWithState({
            isAuthenticated: true, isLoading: false, path: [folder], folder,
            user: { id: 'u1', name: 'User' } as any,
        })
        expect(fixture.nativeElement.querySelector('[data-testid="upup-drive-browser"]')).not.toBeNull()
    })
...
describe('ClientBoxUploaderComponent', () => {
    ...
    function mountWithState(statePartial: Partial<AdapterBrowserState>) {
        ...
        TestBed.overrideComponent(ClientBoxUploaderComponent, {
            set: { providers: [{ provide: BoxService, useClass: FakeService }] },
        })
...
describe('ClientOneDriveUploaderComponent', () => {
    ...
    function mountWithState(statePartial: Partial<AdapterBrowserState>) {
        ...
        TestBed.overrideComponent(ClientOneDriveUploaderComponent, {
            set: { providers: [{ provide: OneDriveService, useClass: FakeService }] },
        })
