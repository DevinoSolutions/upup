...
export class ClientGoogleDriveUploaderComponent implements OnInit, OnDestroy {
    ...
    readonly retryAuth = () => { this.svc.retryAuth() }
    readonly handleSignOut = () => { this.svc.handleSignOut() }
    readonly setPath = (path: Parameters<GoogleDriveService['setPath']>[0]) => { this.svc.setPath(path) }
    readonly handleClick = (file: Parameters<GoogleDriveService['handleClick']>[0]) => { this.svc.handleClick(file) }
