...
export class ClientBoxUploaderComponent implements OnInit, OnDestroy {
    ...
    readonly setPath = (path: Parameters<BoxService['setPath']>[0]) => { this.svc.setPath(path) }
    readonly handleClick = (file: Parameters<BoxService['handleClick']>[0]) => { this.svc.handleClick(file) }
