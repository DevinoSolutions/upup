            ...
            >
                <upup-should-render [when]="true" [isLoading]="isLoading">
                    ...
                    >
                        ...
                        >
                            <span class="upup-text-sm upup-font-medium">{{ providerLabel }}</span>
                            <input
                                type="search"
                            ...
                            @for (file of displayFiles; track file.id) {
                                ...
                                >
                                    ...
                                    @if (file.size != null && !file.isFolder) {
                                        <span class="upup-text-xs upup-opacity-60">{{ formatBytes(file.size!) }}</span>
                                    }
                                </button>
                            }
                        </div>
...
export class ServerModeDriveUploaderComponent implements OnInit, OnDestroy {
    ...
    readonly startAuth = (): void => { this.svc.startAuth() }
    ...
    get errorClass(): string {
        const dark = this.store.isDark()
        return cn('upup-p-4 upup-text-sm', dark ? 'upup-text-red-400' : 'upup-text-red-600')
    }
