...
export class DriveBrowserItemComponent {
    ...
    get isFolder(): boolean { return !!this.file.isFolder }
    ...
    get containerClass(): string {
        ...
        if (this.isFileSelected) {
            ...
            if (slotClasses.driveItemContainerSelected) parts.push(slotClasses.driveItemContainerSelected)
        } else {
            parts.push('upup-bg-[#e9ecef00]')
            if (slotClasses.driveItemContainerDefault) parts.push(slotClasses.driveItemContainerDefault)
        }
        return parts.join(' ')
    }
    ...
    get innerClass(): string {
        const slotClasses = this.store.slotOverrides()
        return cn('upup-flex upup-items-center upup-gap-2', slotClasses.driveItemContainerInner)
    }
