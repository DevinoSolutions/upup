                        ...
                        <div class="upup-flex upup-items-center upup-gap-1">
                            @for (p of path; track p.id; let i = $index) {
                                <p
                                    ...
                                    [style.max-width]="(100 / (path?.length ?? 1)) + '%'"
                                    [style.pointer-events]="i === (path?.length ?? 0) - 1 ? 'none' : 'auto'"
...
export class DriveBrowserHeaderComponent {
    ...
    @Input() onSearch: (value: string) => void = () => { /* noop */ }
    ...
    get searchInputClass(): string {
        ...
        return cn(
            ...
            dark ? 'upup-bg-white/5 upup-text-[#6D6D6D] dark:upup-bg-white/5 dark:upup-text-[#6D6D6D]' : '',
