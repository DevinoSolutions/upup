...
<div
  ...
  class={cn('upup-relative upup-flex-1 upup-overflow-hidden upup-rounded-lg', {
    ...
    'upup-border-[#38bdf8] dark:upup-border-[#38bdf8]': $absoluteHasBorder && $dark,
    'upup-border-dashed': !$isDragging,
    // Idle drag-drop hint: pulse the dashed border between a muted slate and the
    // sky accent while the panel is empty and at rest. Border-color only (no
    // width/layout change); paused whenever a drag, file, active source, or
    // add-more flow is in progress.
    'upup-animate-hint-pulse motion-reduce:upup-animate-none':
      $absoluteHasBorder && !$isDragging && !$files.size && !$activeSource && !$isAddingMore,
