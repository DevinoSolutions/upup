...
export interface UseUpupUploadReturn {
  ...
  progress: Writable<{ totalFiles: number; completedFiles: number; percentage: number }>
...
export function useUpupUpload(options: UseUpupUploadOptions): UseUpupUploadReturn {
  ...
  const progress = writable({ totalFiles: 0, completedFiles: 0, percentage: 0 })
  ...
  return {
    ...
    on: (e: string, h: (...args: unknown[]) => void) => core.on(e, h),
    ext: core.ext,
    core,
  }
}
