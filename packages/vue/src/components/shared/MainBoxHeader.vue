...
const isUploading = computed(() => isUploadActive(uploadControlsCtx.upload.uploadStatus))
const isLimitReached = computed(() => limit === filesCtx.files.size)
const cancelText = computed(() =>
    viewCtx.isAddingMore ? tr.cancel : tr.removeAllFiles,
)

function toggleViewMode() {
    viewCtx.setViewMode(viewCtx.viewMode === 'grid' ? 'list' : 'grid')
}
</script>
            ...
            <div class="upup-col-start-3 upup-col-end-5 upup-flex upup-items-center upup-justify-end upup-gap-2 md:upup-col-start-4">
                <ShouldRender :if="filesCtx.files.size > 1">
                    <button
                        ...
                        :title="viewCtx.viewMode === 'grid' ? tr.switchToListView : tr.switchToGridView"
                    >
                        <LayoutListIcon v-if="viewCtx.viewMode === 'grid'" :size="16" />
                        <LayoutGridIcon v-else :size="16" />
                    </button>
                </ShouldRender>
                <ShouldRender :if="!viewCtx.isAddingMore && limit > 1 && !isLimitReached">
                    <button
                        :class="cn(
                        ...
                        @click="viewCtx.setIsAddingMore(true)"
                        :disabled="isUploading || isProcessing"
                    >
