...
// retired." useUpupUpload's onMounted block (packages/vue/src/use-upup-
// upload.ts) wires a small core.on(...) forwarding table to its onXxx
// options ("vue emits" in this file's name means "invokes the callback
// prop" — this composable has no $emit/defineEmits surface, only callback
// options). This pins that table's exact shape and its "no bare error"
// negative, parity with react's use-upup-upload-events.test.ts.
describe('useUpupUpload — core events forward to vue callback options (P6 forwarding table)', () => {
    it('forwards core "files-added" to onFileAdded with its payload', () => {
        const onFileAdded = vi.fn()
        const { result, unmount } = withSetup(() =>
            useUpupUpload({ limit: 5, onFileAdded }),
        )
    ...
    it('forwards core "file-removed" to onFileRemoved with its payload', () => {
        ...
        const { result, unmount } = withSetup(() =>
            useUpupUpload({ limit: 5, onFileRemoved }),
        )
    ...
    it('forwards core "upload-progress" to onUploadProgress with its payload', () => {
        ...
        const { result, unmount } = withSetup(() =>
            useUpupUpload({ limit: 5, onUploadProgress }),
        )
    ...
    it('forwards core "upload-all-complete" to onUploadComplete with its payload', () => {
        ...
        const { result, unmount } = withSetup(() =>
            useUpupUpload({ limit: 5, onUploadComplete }),
        )
        const payload: UploadFile[] = []
