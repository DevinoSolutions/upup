...
export default function useUploaderPanel(): DragDropSnapshot & {
    ...
    const handleDragOver: DragEventHandler<HTMLDivElement> = controller
        ? e => {
              controller.handleDragOver(e as unknown as DragEvent)
          }
        : NOOP
    const handleDragLeave: DragEventHandler<HTMLDivElement> = controller
        ? e => {
              controller.handleDragLeave(e as unknown as DragEvent)
          }
        : NOOP
    ...
    const handlePaste: ClipboardEventHandler<HTMLDivElement> = controller
        ? e => {
              controller.handlePaste(e as unknown as ClipboardEvent)
          }
        : NOOP
