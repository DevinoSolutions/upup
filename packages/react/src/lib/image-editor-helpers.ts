...
import type { ImageEditorOptions, UploadFile } from '@upup/shared'
...
export function blobToUploadFile(
    ...
    // Preserve identity so upload pipeline sees same entry
    uploadFile.id = original.id
    if (original.key) uploadFile.key = original.key
    if (original.relativePath) uploadFile.relativePath = original.relativePath
