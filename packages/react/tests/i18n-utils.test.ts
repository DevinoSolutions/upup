...
describe('formatUiMessage() interpolation', () => {
  ...
  it('replaces single placeholder', () => {
    expect(formatUiMessage('Upload {{count}} files', { count: 5 })).toBe('Upload 5 files')
  })

  it('replaces multiple placeholders', () => {
    expect(formatUiMessage('{{name}} ({{size}} {{unit}})', { name: 'test.txt', size: 10, unit: 'MB' }))
      .toBe('test.txt (10 MB)')
  })

  it('preserves unmatched placeholders', () => {
    expect(formatUiMessage('Hello {{name}}, {{missing}}', { name: 'World' }))
      .toBe('Hello World, {{missing}}')
  })

  it('handles numeric values', () => {
    expect(formatUiMessage('Max {{size}} {{unit}}', { size: 999, unit: 'MB' }))
      .toBe('Max 999 MB')
  })
...
describe('pluralUiMessage()', () => {
  ...
  it('works with filesSelected key', () => {
    expect(pluralUiMessage(baseMessages, 'filesSelected', 1)).toBe(baseMessages.filesSelected_one)
    expect(pluralUiMessage(baseMessages, 'filesSelected', 3)).toBe(baseMessages.filesSelected_other)
  })
