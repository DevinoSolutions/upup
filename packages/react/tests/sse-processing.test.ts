...
import { FileSource, UploadStatus, type UploadFile } from '@upupjs/core'
...
describe('useSSEProcessing', () => {
    ...
    it('opens EventSource with correct URL when processingEndpoint is set', () => {
        ...
        const { result } = renderHook(() =>
            useSSEProcessing({
                processingEndpoint: '/api/processing',
                onFileProcessed,
            }),
        )
        ...
        expect(MockEventSource.instances[0]!.url).toBe(
            '/api/processing?key=test%2Fkey.txt',
        )
    })
    ...
    it('calls onFileProcessed with parsed JSON when message arrives', () => {
        ...
        act(() =>
            MockEventSource.instances[0]!.emit(
                '{"status":"done","url":"https://cdn.example.com/doc.pdf"}',
            ),
        )
