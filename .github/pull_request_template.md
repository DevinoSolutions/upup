...
- [ ] I have added a changeset (`pnpm changeset`) if this PR changes any publishable `@upup/*` package

## Test-foundation checklist (delete rows that cannot apply)

- [ ] UI change: landed in `@upup/react` first, ported DOM-identically, and `parity-fixtures.json` was regenerated via `UPDATE_PARITY=1` + reviewed like code (never in CI)
