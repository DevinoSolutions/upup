...
| Cloud-drive server-transfer   | `apps/e2e-test/drive-sandbox/box-dropbox-server-transfer.spec.ts` (env-gated `UPUP_DRIVE_SANDBOX=1` + Box/Dropbox sandbox secrets + MinIO)                | the same Box/Dropbox sandbox creds proven through `@upup/server`'s HTTP surface (route dispatch → drive auth → drive→S3 transfer) into a real MinIO bucket, byte-exact sha256; skips green without creds/MinIO, reds on a configured-but-broken token  | `pnpm --filter @upup/e2e-test test:e2e:drive-sandbox` (nightly-only in CI)            |
...
vitest client-direct suite and a Playwright HTTP-surface layer (Box/Dropbox
through `@upup/server` into MinIO). Nothing publishes
